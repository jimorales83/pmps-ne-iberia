...
This repository uses two simple licences because research content and executable code are reused in different ways.

## Code

The R scripts in `R/` and the OxCal model scripts (`*.oxcal`) in `oxcal/` are licensed under the MIT License. See `LICENSES/MIT.txt`.

The OxCal software itself is not distributed by this repository and is not covered by this licence.

## Research materials

Unless a source-specific notice states otherwise, the authors' data, documentation, tables, exported analytical outputs and visual materials are licensed under the Creative Commons Attribution 4.0 International licence (CC BY 4.0). See `LICENSES/CC-BY-4.0.txt`.
